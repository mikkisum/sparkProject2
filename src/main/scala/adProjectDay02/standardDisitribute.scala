...
object standardDisitribute {
  def main(args: Array[String]): Unit = {
   // standardDistribute("ispname")
    standardDistribute("devicetype")


  }

  //根据给定的key值给出指标
  def standardDistribute(key:String)={
    //建立连接
    ...
    val data: RDD[(String, List[Double])] = df.map(row => {
      ...
      //key 值
      var kString=""
      var k = row.getAs[Int](s"$key")
     // if (k=="未知"){k="其他"}
      if(k==1){kString="手机"}
      else if(k==2){kString="平板"}
      else{kString="其他"}
      //创建3个值对应处理9个指标
